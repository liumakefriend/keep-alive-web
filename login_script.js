...
(async () => {
  console.log("🚀 启动隐身浏览器 (小窗口模式)...");
  
  const browser = await puppeteer.launch({
    headless: "new", // 使用新版无头模式
    // 关键修改 1: 设置为 null，让视口大小跟随 window-size 自动调整
    defaultViewport: null, 
    args: [
      '--no-sandbox', 
      '--disable-setuid-sandbox',
      // 关键修改 2: 将分辨率从 1920,1080 改为 1024,768
  ...
  // 1. 访问登录页面
  const loginUrl = 'https://betadash.lunes.host/servers/53360'; 
  ...
  console.log("🛡️ 等待 10 秒，让 Cloudflare 十秒盾自动通过...");
  ...
  // 注意：Puppeteer 的 evaluate 方法比直接 match text 更准确
  const data = await page.evaluate(() => {
     ...
     result.UUID = extract(/UUID\s+([a-z0-9]+)/i);
     result.Plan = extract(/Plan\s*\n\s*(.+)/i);
     result.Memory = extract(/Memory\s*\n\s*([0-9]+\s*MB)/i);
     result.CPU = extract(/CPU\s*\n\s*([0-9]+%)/i);
